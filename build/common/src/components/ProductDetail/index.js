...
class ProductDetail extends React.Component {
  ...
  renderPrice = (product) => {
    ...
    const priceAvalidable = Math.round(isDiscountAvalidable ? (selectProduct.price / (1 - (selectProduct.discountPercentage / 100))) : selectProduct.price);
      ...
      <>
        {isDiscountAvalidable ?
          <div>
            ...
            <p className='price'>
              {priceFormatter(selectProduct.price)}
      ...
      <Content>
        ...
        <Detail>
          <ContentSlide width='65%'>
            ...
            {isProductComplementation && <FrequentlyBuyed products={product.relatedProducts} addToWishlist={addToWishlistId} responsive={false} productTarget={product} addProductToCart={this.handlerAddComplementations.bind(this)} />}
            {seeTire &&
              <TireGuide>
                <p>Guía de llanta</p>
                <TireGuideImg>
          ...
          <InfoContainer>
                  ...
                  {this.props.selectedCar.serie &&
                    <CompatibleCar>
                      <div className="actual-car-check">
                        <div className={` ${isCompatibility ? 'squaredThree ' : 'squaredThree squaredThree--not'}`}>
                          <input type="checkbox" value="None" id="squaredThree" name="check" checked disabled className="css-checkbox" />
                        ...
                        <div className="data-car">
                          ...
                          <p><strong>{this.props.selectedCar.model.name}</strong> - {this.props.selectedCar.year}</p>
                        </div>
                      </div>
              ...
              {seeTire &&
                <TireDataContent>
                  <TireDataList>
                    ...
                    <TireDataListItem>
                      <p>Marca</p>
                      <img width="80%" src={`/static/images/tires/${product.productBrand != null ? product.productBrand.toLowerCase() : ''}.png`} />
                    </TireDataListItem>
                    <TireDataListItem>
                      <p>Uso</p>
                      <p>{product.type}</p>
                  ...
                  <ComplementStyle key={i}>
                    <label className="checkbox-button">
                      <div className="checkbox-button__label">
                        ...
                        <span style={{ flex: '1 1', textAlign: 'left' }}>{priceFormatter(complement.priceWithTax)}</span>
                        <span>
                          <label>
                            <input type="checkbox" className="checkbox-button__input" id="choice1-1" name="choice1" onChange={(e) => this.handleChangeKit(e, complement.priceWithTax)} />
            ...
            {product.stock <= 5 &&
              <CountLabel>Cantidad: &nbsp;<span className="font-italic">   {product.stock + ' en existencia'} </span></CountLabel>
            }
            <ActionProductContent>
              <CountContent>
                <ContentCount>
              ...
              <SpecificationsModal>
                ...
                {!!(product.attributevalues && product.attributevalues.length) && (
                  getUnique(product.attributevalues, 'value').map((item, i) => (
                    !(item.value.split(/\s/)[0].includes("0")) && <p key={i}><strong>{item.value.split(/\s/)[0]}</strong> &nbsp; {item.value.substr(item.value.indexOf(" ") + 1)}</p>
                  ))
                )}
          ...
          {isProductComplementation && <FrequentlyBuyed products={product.relatedProducts} productTarget={product} addToWishlist={addToWishlistId} responsive={true} addProductToCart={this.handlerAddComplementations.bind(this)} />}
        </Detail>
        <Modal
          ...
          <ModalSeriesContainer>
            <SearchContent>
              <div>
                ...
                />
                <input type="search" placeholder="Busca el modelo serie año" onChange={(e) => this.handleModalCompatibilitySearch(e, models)} />
              </div>
            </SearchContent>
            ...
            <ContentModels>
              ...
              <YearList>
                ...
                {this.getCompatibilityYears(searchValue ? showedSeries : models).map((year, i) => (
                  <YearTabItem key={i} selected={selectedYear === year} onClick={() => this.setState({ selectedYear: year })}>
                    {year}
                  </YearTabItem>
                ))}
              </YearList>
