			...
			<Card>
						...
						<div>
                          ...
                          {attr.name} {attr.value}
							...
							<div className='hide-mobile price-content'>
								...
								<p className='bold price-total'>{priceFormatter(Math.round(calculardescuentos ? (priceWithTax - ((priceWithTax * discountPercentage) / 100)) : priceWithTax) * qty)}</p>
								<p className='bold italic price-unitary'>{priceFormatter(Math.round(calculardescuentos ? (priceWithTax - ((priceWithTax * discountPercentage) / 100)) : priceWithTax))} c/u</p>
							</div>
						</div>
					</Detail>
				</Content>
				<DataMobile>
					...
					<div className="data-info">
						<p>Cantidad</p>
						<Select options={options} styles={customStyles} placeholder="x1" placeholder={qty}
								onChange={(e) => addQtyToDetail(item, e.value)}/>
					</div>
					<div className="data-info">
						<p>Precio</p>
				...
				{item.productCategory && item.productCategory.name == "LLANTAS" &&
				<label className="tireNotify">Asegúrate de elegir tu llanta con el índice correcto. Tu seguridad es
					nuestro compromiso.</label>}
			</Card>
		</>
	)
}
